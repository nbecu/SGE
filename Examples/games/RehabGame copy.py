...
settleAction= myModel.newModelAction_onCells(lambda cell: Bird.newAgentOnCellWithModelView(cell),(lambda cell: cell.value('biomass')>=2))
...
def reproduce(aBird):
    ...
    for i in range(aBird.value('nb reproduction')):
        Chick.newAgentOnCellWithModelView(aBird.cell)
