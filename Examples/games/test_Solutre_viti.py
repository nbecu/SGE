...
def execEvent():
    ...
    Touriste.newAgentsAtCoordsWithModelView(int(eventLine["nbTouristes"].squeeze()+bonusTouriste.value),reserve,1,1)
...
def execFirstEvent():
    ...
    myModel.newPopUp(eventLine['Nom'].squeeze(),eventLine["Descriptif"].squeeze()+" Nombre de touristes cette année :"+str(eventLine["nbTouristes"].squeeze()))
    Touriste.newAgentsAtCoordsWithModelView(int(eventLine["nbTouristes"].squeeze()),reserve,1,1)
...
Embuissonnement=myModel.newModelAction([lambda: Buisson.newAgentsAtRandomWithModelView(3,Plateau,condition= lambda aCell: aCell.value("zone")=="Roches")])
...
PlayPhase=myModel.newPlayPhase("Phase 1 : Aménager le territoire",[Viticulteur,Elu,Habitant,Naturaliste,Tourisme])
...
def placeInitHexagones():
    ...
    print(f"DEBUG: Placing initial hexagones. Current player: {myModel.currentPlayerName}")
    print(f"DEBUG: Total hexagones: {len(hexagones.getEntities())}")
    
    # Force layout recalculation before positioning
    QApplication.processEvents()
    
    for aHex in hexagones.getEntities():
        print(f"DEBUG: Hex {aHex.id}: nom='{aHex.value('nom')}', placed={aHex.value('placed')}")
        if aHex.value("placed")==False:
            if aHex.value("nom")=="Vigne du plateau":
                print(f"DEBUG: Placing Vigne du plateau")
                aHex.setValue("placed",True)
                aHex.moveTo(Plateau.getCell(8,4))
                # Force view update after moving
                if hasattr(aHex, 'view') and aHex.view:
                    QApplication.processEvents()  # Ensure layout is applied
                    aHex.view.getPositionInEntity()
                    aHex.view.show()
                
            if aHex.value("nom")=="Caveau du plateau":
                print(f"DEBUG: Placing Caveau du plateau")
                aHex.setValue("placed",True)
            ...
            if aHex.value("nom")=="Chambre d'hôtes du plateau":
                print(f"DEBUG: Placing Chambre d'hôtes du plateau {n+1}")
                aHex.setValue("placed",True)
                aHex.moveTo(cells[n])
                # Force view update after moving
                if hasattr(aHex, 'view') and aHex.view:
