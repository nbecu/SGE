...
|------|-------------------------|----------------------|
| **47** | Create automatic recording of simulation state at each round | **Implémenté** (recovery à chaque phase, `enableRecoverySystem()`, `_recovery_states/`, rotation `recoveryStatesMax`) |
| **48** | Create a recovery system for simulation in case it shuts down (could use `updateAtMaj` function) | **Implémenté** (recovery au démarrage si fichiers présents, proposition « Restore » ; suppression des fichiers à la fermeture normale) |
| **49** | Add item in SGModel menu bar to record state of the world, and use saved state as initialization for new simulation | **Implémenté** (menu World state : Save current state, Load state as initial state, Save whole simulation, Replay a simulation) |
| **50** | Add the possibility to export gameAction logs | **Déjà implémenté** (voir §2) |

---

## 1bis. Item 7 – Mode distribué (terminé)
...
- **Item 7 – Mode distribué** : MQTT backward/forward synchro + recovery state (demande/réponse).
  ...
  - **Recovery en mode distribué** : **Fait**. Si une instance plante, à son redémarrage elle choisit « Reconnect to a session already started », envoie une **demande via MQTT** (topic `game_recovery`) ; une instance encore en partie répond avec le dernier world state, `assigned_player_name`, seed, état RNG complet et liste des joueurs connectés. L’instance qui se reconnecte applique l’état dans `initBeforeShowing()`, ré-enregistre le joueur et s’abonne aux topics registration pour recevoir les heartbeats. Pas de proposition de recovery local en mode distribué (skip si `isDistributed()`).

---

## 1bis2. Recovery distribué – Rejoindre la session après crash

**Problème** : Pour faire un recovery, l’instance qui a planté doit d’abord **rejoindre la même session** (même `session_id`, souscription aux topics MQTT, etc.) avant de pouvoir demander le dernier world state. Or, les sessions **démarrées** (`state == 'started'`) sont exclues de la liste « Available sessions » : l’instance qui redémarre ne voit pas la session « en cours » et ne peut pas la rejoindre.

**Décisions / précisions** :

- **Qui peut « Reconnect »** : **Toute instance** qui se connecte et qui entre dans le mode « Reconnect to a session in progress » (ou « to a session already started »). Pas limité à l’instance qui a crashé.

- **Assigned player name** : **Pas de persistance côté instance qui se reconnecte.** Les instances restées dans la session savent quel joueur est déconnecté (elles ont reçu `session_player_disconnect` et ont mis à jour `connected_players`). Elles peuvent **informer l’instance qui se reconnecte** du `assigned_player_name` (ex. dans la réponse à la demande de recovery, ou via un message dédié : « ce client_id se reconnecte en tant que Player 2 »). À implémenter : mémoriser côté instances « en vie » le lien (client_id → player_name) pour les joueurs déconnectés, et inclure cette info dans la réponse recovery ou dans un topic de reconnexion.

- **Session fermée entre-temps** : **Rien de particulier à afficher.** Si la session a été fermée entre-temps, elle n’apparaît simplement pas dans la liste des « sessions in progress » / « sessions already started ». Pas de message d’erreur dédié à prévoir pour ce cas.

- **Accès à la liste « sessions in progress »** : **Option A retenue.** Un **troisième mode** « Reconnect to a session already started » à côté de « Create » et « Join », qui affiche ensuite uniquement les sessions `started` et un bouton « Reconnect ».  
  **Condition d’affichage** : ce 3ᵉ mode ne doit **apparaître que si** le modèle a appelé `enableRecoverySystem(True)`. Sinon, seuls Create et Join sont proposés. (À l’implémentation : le dialogue de connexion doit connaître cet état, ex. via la config distribuée ou un attribut du modèle.)

**Flux cible** (une fois la session identifiée) : connexion broker, `session_id` = session choisie, souscription aux topics ; l’instance en reconnexion envoie une demande de recovery (ou annonce qu’elle se reconnecte) ; une instance encore en partie répond avec le dernier world state **et** le `assigned_player_name` à utiliser ; application du state, puis envoi de `session_player_registration` pour ce joueur afin que les autres voient à nouveau « Player X (Connected) ».

**Topic MQTT recovery** : **Un seul topic dédié** au recovery (ex. `game_recovery` ou dans les session topics). Les messages sur ce topic distinguent demande et réponse via un champ dans le payload (ex. `"type": "request"` vs `"type": "response"`), éventuellement avec un `request_id` pour associer une réponse à une demande. Pas de séparation en deux topics request/response.

**Plan d’implémentation** (ordre suggéré, bonnes pratiques SGE à respecter : code/commentaires/docstrings en anglais, `camelCase` pour les méthodes, `snake_case` pour variables/fonctions, séparation developer / modeler methods, conventions de nommage des méthodes modeler) :
1. Dialogue : 3ᵉ mode « Reconnect to a session already started » (visible seulement si `enableRecoverySystem(True)`), liste des sessions `started`, bouton « Reconnect ».
2. Un seul topic recovery : définition du topic, format payload (type request/response, request_id, state, assigned_player_name).
3. Côté reconnecting : envoi demande, réception réponse, application state, ré-enregistrement joueur ; timeout + fallback recovery local.
4. Côté instances en partie : mémorisation client_id → player_name pour les déconnectés ; réponse à la demande recovery (state + assigned_player_name).
5. **Gel de la partie en cas de déconnexion** (§1ter) : détection via `playerDisconnected` ; option modeler `freeze_game_on_player_disconnect` ; désactivation game actions + next step (ou overlay) quand gelé ; option « Reprendre sans attendre » (si autorisée par le modeler) ; lever le gel à la reconnexion (réapparition dans `connected_players`).
...
### 6.3 Ordre de travail suggéré (état d’avancement)

| # | Objectif | Statut |
|---|----------|--------|
| 1 | Spécifier et implémenter le **format world state** + sérialisation/désérialisation (lecture/écriture fichier) | **Fait** (SGStateSnapshot, format JSON, .json.gz) |
| 2 | Implémenter **enregistrement manuel** + item de menu “Save state” (49) | **Fait** (Save current state, Load state as initial state) |
| 3 | Implémenter **chargement comme état initial** + item de menu “Load state” (49) | **Fait** |
| 4 | Ajouter **enregistrement automatique à chaque phase** (47) (option + appel dans le cycle) | **Fait** (`enableRecoverySystem()`, `_recovery_states/`, rotation `recoveryStatesMax`, paramètre dans SGSGE_parameters) |
...
| 8 | **Mode distribué** : écriture disque sur chaque instance ; backward/forward diffusés via MQTT (topic `game_step_navigation`) ; recovery avec demande du dernier world state via MQTT | **Fait** : backward/forward MQTT **fait** ; recovery distribué **fait** (topic `game_recovery`, request/response, state + assigned_player_name + seed + rng_state + connected_players ; Reconnect mode dans le dialogue ; gel/dégel sur déconnexion/reconnexion ; Connection Status à jour des deux côtés) |
...
- **Tiles / images** : reporté hors chantier — ajouter dans FUTURE_PLAN : *Correct the way images are associated to tiles, so that each tile knows which image it has* (problème d’implémentation actuelle des tiles, pas du snapshot).
...
## 10. État actuel de l’implémentation (résumé)

- **Format** : `mainClasses/SGStateSnapshot.py` — `build_snapshot_from_model`, `apply_snapshot_to_model`, `read_snapshot_from_file`, `write_snapshot_to_file`, `write_snapshots_array_to_file`, `load_snapshot_array_from_json_gz`. Fichiers `.json` ou `.json.gz`, `format_version` en tête.
- **Recovery** : répertoire **`_recovery_states/`** (un dossier, pas de sous-dossier par modèle) ; noms de fichiers **`<nom_modèle>_state_r<R>_p<P>_<timestamp>.json.gz`**. Rotation : `recoveryStatesMax` (paramètre SGE dans `SGSGE_parameters.py` : `RECOVERY_STATES_MAX` / `recovery_states_max`). API modeler : **`myModel.enableRecoverySystem(True)`**. À la fermeture normale, les fichiers recovery de ce modèle sont supprimés ; au démarrage, si des fichiers existent, proposition « Restore the last simulation? » (sauf en mode distribué : en distribué, l’utilisateur utilise « Reconnect to a session already started » pour recovery via MQTT).
- **Recovery distribué** : topic **`game_recovery`** (request/response). L’instance qui se reconnecte envoie une requête ; une instance en partie répond avec state (snapshot), `assigned_player_name`, seed, état RNG complet (`rng_state`) et `connected_players`. Application du state dans `initBeforeShowing()` ; ré-enregistrement joueur (`publishPlayerRegistrationReconnect`) et souscription aux topics registration pour recevoir les heartbeats ; mise à jour de Connection Status (instance A : refresh widget lors de playerConnected/Disconnected ; instance B : merge `connected_players` depuis la réponse + souscription pour garder les autres en « Connected »).
- **Replay / simulation complète** : répertoire **`saved_simulations/`** ; menu « Save whole simulation », « Replay a simulation », « Exit replay ». En mémoire : **`_simulation_snapshots`** (limite **`SIMULATION_SNAPSHOTS_MAX`** dans `SGSGE_parameters.py`). API : **`enableAutoSaveSimulationOnClose(confirm=True)`**, **`enableAutoSaveSimulationOnEndGame(True)`**.
- **Backward / forward** : **`_undo_stack`** / **`_redo_stack`** ; **`pushStateAfterEvent()`** après chaque game action et chaque nextStep ; **`_backward_max_states`** pour la taille de la pile.
- **Menu World state** (Settings) : Save current state, Load state as initial state, Save whole simulation, Replay a simulation, Exit replay. Pas d’item à cocher pour le recovery (activation uniquement par script via `enableRecoverySystem()`).
- **Exemple** : `examples/syntax_examples/ex_stepback_and_recovery.py` ; **CarbonPolis** : `examples/games/CarbonPolis.py` avec `myModel.enableRecoverySystem(True)` avant `myModel.launch()`.
