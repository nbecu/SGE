...
#Tests dans exProductionAndConsumption_copy.py :
# ajouter une API qui permet au modeler de demander à ce que les items du menu de graph de la fentre de principale soient masqués (comme ça, si il veut, il ne peut y avoir que les graphs presets qui apparaissent dans le menu graph de la fenetre principale)
# # dans le menu graph de l'interface principale, ajouter  l'icone du type de graph devant le nom du graph preset (ex: devant "Production Unit mean energy", mettre l'icone d'un graph linéaire)
# # le nom du graph preset apparait correctement dans le menu grap, mais par contre l'intitulé du graph ne correspond (dans la fenetre de graph) ne correspond pas au nom preset 
...
myModel.hideDefaultGraphMenuItems('linear')
