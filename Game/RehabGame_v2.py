...
myModel.timeManager.newGamePhase('Harvesters',[Player1,Player2])
myModel.timeManager.newGamePhase('Manager',[Player1,Player2])

# ajout de la ModelPhase
aCell= aGrid.getRandomCell()
harvestAction = myModel.newModelAction([lambda : aCell.setValue('Resource',str((int(aCell.value('Resource')) -(len(aCell.getAgents())) )) ) ])
myModel.timeManager.newModelPhase(harvestAction)
...
i1=DashBoard.addIndicator("sumAtt",'cell',color = Qt.red, attribute='Resource')
i2=DashBoard.addIndicator("avgAtt",'cell',color = Qt.red, attribute='Resource')
