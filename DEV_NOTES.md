...
- **Description** : Corrections de 8 problèmes sur 10 identifiés dans le diagnostic architectural Mai 2026, et mise en place d'une suite de tests pytest complète.
- **Fichiers modifiés** :
  - `mainClasses/SGModel.py` (30 star imports → explicites, imports morts supprimés, getAllGameSpaces() ajouté, positionAllAgents() délègue à repositionView(), commentaires orphelins nettoyés)
  - `mainClasses/SGAgent.py` (repositionView() ajouté, ValueError clarifiée)
  - `mainClasses/SGTimeManager.py` (nextPhase() décomposée en 3 méthodes privées)
  - `mainClasses/SGExtensions.py` (classe SGColors créée, monkey-patching simplifié en boucle)
  - `mainClasses/AttributeAndValueFunctionalities.py` (clearHistory() ajouté)
  - `mainClasses/SGSGE.py` (SGColors ajouté à __all__)
  - `tests/conftest.py` (nouveau — QApplication session-scoped)
  - `tests/test_model_setup.py` (nouveau — 23 tests de régression)
  - `tests/test_neighborhood.py` (réécrit avec vrais assert)
  - `tests/manual/README.md` (nouveau — instructions scripts manuels)
  - `README_modeler.md` (SGColors, clearHistory() documentés)
  - `CONTEXT_SGE_FOR_CHATBOT.md` (SGColors, getAllGameSpaces(), clearHistory(), règle imports explicites)
- **Problèmes résolus** :
  1. P1 — 30 star imports remplacés par imports explicites dans SGModel.py
  2. P2 — `getAllGameSpaces()` créé pour unifier `gameSpaces` et `TextBoxes`
  3. P4 — `nextPhase()` décomposée : `_advanceCounters()`, `_updateCurrentPlayer()`, `_executeAndRefresh()`
  4. P5 — 4 imports morts supprimés dans SGModel.py
  5. P6 — `SGAgent.repositionView()` : SGModel ne touche plus les views directement
  6. P7 — `SGColors` créé comme source unique de vérité pour ~80 couleurs nommées
  7. P8 — `clearHistory(before_round=None)` ajouté sur AttributeAndValueFunctionalities
  8. P9 — Commentaires orphelins supprimés, ValueError clarifiée dans SGAgent
- **Problèmes non traités** :
  - P3 — Mixin couplé à `model.timeManager` : effort élevé, reporté
  - P10 — SGGameSpaceSizeManager : état per-instance justifié (setters utilisés), pas de correction nécessaire
- **Tests** : Suite pytest installée et organisée (54 tests, 3 fichiers, conftest.py, @pytest.mark.parametrize). Note : access violation à l'exit PyQt5 est un artefact cosmétique connu, tous les tests passent.
- **Durée** : 3 sessions (Mai 2026)

### Janvier 2025 - Corrections menu Settings et synchronisation Cursor Position (TERMINÉ)
- **Statut** : ✅ Terminé
- **Description** : Corrections de l'ordre des items du menu Settings, correction de la duplication de "Entity Tooltips", et correction de la synchronisation de l'état de "Cursor Position" en mode distribué
- **Fichiers modifiés** :
  - `mainClasses/SGModel.py`
- **Problèmes résolus** :
  1. Ordre des items du menu Settings non optimal
  2. "Entity Tooltips" apparaissait trois fois (duplication)
  3. En mode distribué, "Cursor Position" n'était pas coché mais l'affichage était actif (incohérence)
- **Solutions implémentées** :
  1. Réorganisation de l'ordre des items du menu Settings dans `createMenu()` :
     - "Entity Tooltips" (en premier)
     - "Enhanced Grid Layout"
     - "Themes"
     - "Export GameAction Logs"
     - "Cursor Position" (en dernier)
  2. Correction de la duplication de "Entity Tooltips" : ajout d'une vérification dans `createTooltipMenu()` pour ne créer le menu que s'il n'existe pas déjà
  3. Synchronisation de l'état de "Cursor Position" :
     - Stockage de la référence à l'action dans `self.cursorPositionAction`
     - Initialisation explicite de l'action à `False`
     - Synchronisation de l'état de l'action avec `isLabelVisible` dans `showCursorCoords()`
     - Vérification et synchronisation dans `initUI()` et `initBeforeShowing()` pour garantir la cohérence en mode distribué
- **Notes techniques** :
  - L'action `cursorPositionAction` est maintenant stockée comme attribut d'instance pour permettre la mise à jour de son état
  - La synchronisation est effectuée à plusieurs points clés (initialisation, changement d'état, avant affichage) pour garantir la cohérence

### Janvier 2025 - Améliorations de l'affichage de fin de partie (EN PLANIFICATION)
- **Statut** : 📋 Planification complète - Prêt pour implémentation
- **Description** : Amélioration de l'UX pour la fin de partie avec système flexible de bannière/highlight/animation, affichage automatique du widget endGameRule, coche dès détection de condition, et compteur dynamique de rounds/phases restants
- **Documentation détaillée** : Voir `docs/guides/END_GAME_DISPLAY_IMPROVEMENTS_PLAN.md` pour le plan complet
- **Fichiers concernés** (prévus) :
  - `mainClasses/SGEndGameRule.py` (API modeler, bannière, highlight, animation, renommage méthode)
  - `mainClasses/SGEndGameCondition.py` (coche dès détection, compteur dynamique)
  - `mainClasses/SGTimeManager.py` (activation bannière/highlight)
  - `mainClasses/SGModel.py` (widget bannière, paramètres newEndGameRule)
  - Documentation (README_modeler.md, README_developer.md, CONTEXT_SGE_FOR_CHATBOT.md)
- **Problèmes identifiés** :
  - Fenêtre vide bizarre quand `showEndGameConditions()` non appelé
  - Coche ✓ apparaît trop tard (après delay_rounds)
  - Utilisateur non informé du temps restant (rounds/phases)
  - Fin de partie pas assez visible (seulement coche ✓)
- **Solutions prévues** :
  - Affichage automatique du widget quand condition détectée
  - Coche ✓ dès détection + compteur dynamique de temps restant
  - Système flexible bannière + highlight + animation (configurable par modeler)
  - Renommage `showEndGameConditions()` → `displayEndGameConditions()`
  - API modeler complète avec méthodes SET/GET et `setEndGameDisplay()`
- **Phases d'implémentation** :
  1. ✅ Corrections de base (widget invisible par défaut, affichage automatique)
  2. ⏳ Affichage coche et compteur (dès détection, texte dynamique)
  3. ⏳ Système bannière + highlight (widget bannière, styles dynamiques, animation)
  4. ⏳ API modeler complète (paramètres, méthodes SET/GET, renommage)
  5. ⏳ Documentation et tests (docstrings, README, exemples)
- **Valeurs par défaut** :
  - `endGameDisplayMode='highlight'` (discret par défaut)
  - `endGameAnimationEnabled=False` (désactivé par défaut)
  - `endGameBannerText='Game over'`
  - `endGameBannerColor=Qt.red`
- **Compatibilité** : Tous les exemples existants fonctionnent (renommage méthode avec alias pour compatibilité)
- **Durée estimée** : Session complète de développement

### Décembre 2024 - Documentation SGTile dans README et CONTEXT (TERMINÉ)
- **Statut** : ✅ Terminé et validé
- **Description** : Ajout de la documentation de SGTile dans README_developer.md, README_modeler.md et CONTEXT_SGE_FOR_CHATBOT.md pour compléter la documentation des entités principales SGE
- **Fichiers concernés** : 
  - `README_developer.md` (ajout SGTile/SGTileView dans section Model-View Architecture, isTileType dans Type Identification Attributes)
  - `README_modeler.md` (ajout section "Tile Movement and Management")
  - `CONTEXT_SGE_FOR_CHATBOT.md` (ajout SGTile/SGTileView dans architecture Model-View, isTileType, méthodes factory, newFlipAction, section tiles movement)
- **Problèmes rencontrés** : 
  - SGTile non documenté au même niveau que SGAgent et SGCell
  - Manque d'informations sur les tiles dans la documentation pour développeurs et modelers
- **Solutions appliquées** : 
  - Ajout concis de SGTile et SGTileView dans la section Model-View Architecture de README_developer.md
  - Ajout de isTileType dans la section Type Identification Attributes
  - Ajout d'une section concise "Tile Movement and Management" dans README_modeler.md
  - Ajout des méthodes factory pour tiles (newTileType, newTileOnCell) dans CONTEXT_SGE_FOR_CHATBOT.md
  - Ajout de newFlipAction dans les gameActions
  - Ajout d'une sous-section sur le mouvement des tiles dans les méthodes de déplacement
  - Documentation économe et efficace pour ne pas alourdir les fichiers
- **Résultat** : SGTile maintenant documenté au même niveau que SGAgent et SGCell dans toute la documentation SGE, informations essentielles disponibles pour développeurs et modelers
- **Durée** : Session de documentation (Décembre 2024)

### Début Novembre  2025 - GS_Aspect System Development (TERMINÉ)
- **Statut** : ✅ Terminé et validé
- **Description** : Implémentation complète du système `gs_aspect` pour uniformiser la gestion des styles dans tous les GameSpaces de SGE, avec système de thèmes prédéfinis et personnalisés, persistance des thèmes custom, et refactorisation de `onTextAspectsChanged()` pour réduire la duplication de code
- **Fichiers concernés** : 
  - `mainClasses/SGAspect.py` (classe centrale avec thèmes prédéfinis, méthodes `applyToQFont()`, `getStyleSheetForColorAndDecoration()`, `applyToQLabel()`)
  - `mainClasses/SGGameSpace.py` (méthodes modeler communes, helper `_applyAspectToLabel()`, découverte dynamique des thèmes, support background_image)
  - `mainClasses/SGExtensions.py` (méthode utilitaire `mapAlignmentStringToQtFlags()`)
  - `mainClasses/SGModel.py` (factory methods avec paramètres de style passant par setters, méthodes `applyThemeConfig()` et `applyLayoutConfig()`, menu "Change window background color")
  - `mainClasses/SGTextBox.py` (refactorisation `onTextAspectsChanged()` avec helpers)
  - `mainClasses/SGDashBoard.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/SGProgressGauge.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/SGEndGameRule.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/SGUserSelector.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/SGLabel.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/SGButton.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/SGTimeLabel.py` (refactorisation `onTextAspectsChanged()`)
  - `mainClasses/theme/SGThemeConfigManager.py` (persistance thèmes custom, chargement au démarrage)
  - `mainClasses/theme/SGThemeCustomEditorDialog.py` (création/édition thèmes custom, persistance)
  - `mainClasses/theme/SGThemeEditTableDialog.py` (assignment de thèmes, découverte dynamique, "Apply to All...")
  - `mainClasses/theme/SGThemeCodeGeneratorDialog.py` (génération code Python pour promouvoir thèmes custom)
  - `mainClasses/theme/SGThemeConfigManagerDialog.py` (interface améliorée)
  - `examples/syntax_examples/ex_game_space_style_various_syntax.py` (exemple des différentes syntaxes)
  - `README_modeler.md` (section "Styling GameSpaces")
  - `README_developer.md` (section "GS_Aspect System Architecture")
  - `CONTEXT_SGE_FOR_CHATBOT.md` (section 23 "GS_Aspect System Architecture")
- **Problèmes rencontrés** : 
  - Duplication massive de code dans `onTextAspectsChanged()` entre GameSpaces
  - Contournement du système `gs_aspect` par paramètres de style dans constructeurs
  - Découverte dynamique des thèmes prédéfinis complexe (inspect module)
  - Persistance des thèmes custom non implémentée
  - Problèmes de hauteur/word-wrap dans SGTextBox
- **Solutions appliquées** : 
  - Refactorisation complète de `onTextAspectsChanged()` avec helpers centralisés (`_applyAspectToLabel()`, `mapAlignmentStringToQtFlags()`, `applyToQFont()`, `applyToQLabel()`)
  - Réduction de duplication : ~60-70% de code en moins
  - Factory methods modifiées pour utiliser setters (tous les styles passent par `gs_aspect`)
  - Découverte dynamique simplifiée des thèmes prédéfinis (appel direct + filtrage)
  - Persistance des thèmes custom dans `theme_config.json` avec chargement au démarrage
  - Remplacement SGTextBox par SGTextBoxLargeShrinkable (nouvelle implémentation robuste)
  - Support background_image pour tous les GameSpaces
  - Système de thèmes avec `text_aspects` différenciés pour title1/title2/text1/etc.
  - Méthodes modeler `applyThemeConfig()` et `applyLayoutConfig()` avec comportement retardé
  - Menu "Change window background color" pour personnaliser la fenêtre principale
  - Refactoring positionnement des dialogs avec fonction utilitaire `position_dialog_to_right()`
  - Documentation complète dans README et CONTEXT
- **Résultat** : Système `gs_aspect` complet et uniformisé pour tous les GameSpaces, réduction massive de duplication de code, système de thèmes fonctionnel avec persistance, documentation à jour pour intégration de nouveaux GameSpaces
- **Durée** : Session complète de développement (Janvier 2025)
- **Commits** : Multiple commits sur branche `dev_generaliazing_gs_aspects`

### D├®cembre 2024 - Refactoring SGModel + Extraction MQTT (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Fichiers concern├®s** : 
  ...
  - `mainClasses/gameAction/SGAbstractAction.py` (mise ├á jour r├®f├®rence MQTT)
  - `examples/games/MQTT_GameExample_Player1.py` (test validation)
  - `FUTURE_PLAN.md` (mise ├á jour chantiers termin├®s)
  - `CONTEXT_SGE_FOR_CHATBOT.md` (mise ├á jour organisation m├®thodes)
...
- **Solutions appliqu├®es** : 
  ...
  - Mise ├á jour toutes r├®f├®rences MQTT vers SGMQTTManager
  - Ajout m├®thodes placeholder pour visibilit├® IDE outline
  - Documentation mise ├á jour (FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
### D├®cembre 2024 - Renommage SGEntityDef ÔåÆ SGEntityType (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Fichiers concern├®s** : 
  ...
  - `mainClasses/SGEntity.py` (attribut classDef ÔåÆ type)
  - `mainClasses/SGCell.py` (attribut classDef ÔåÆ type)
  - `mainClasses/SGAgent.py` (attribut classDef ÔåÆ type)
  - `mainClasses/AttributeAndValueFunctionalities.py` (classDef ÔåÆ type)
  - `mainClasses/SGLegendItem.py` (classDef ÔåÆ typeDef, entityType() ÔåÆ category())
  - `mainClasses/SGFormatDataHistory.py` (classDef.entityName ÔåÆ type.name)
  - `mainClasses/SGPlayer.py` (targetEntDef ÔåÆ targetType)
  - `mainClasses/SGControlPanel.py` (targetEntDef ÔåÆ targetType)
  - `tests/` (tous les fichiers de test mis ├á jour)
  - `examples/A_to_Z_examples/exStep3_2.py` (newAgentSpecies ÔåÆ newAgentType)
- **Probl├¿mes rencontr├®s** : 
  - Cha├«ne de cr├®ation des LegendItems dans ControlPanels cass├®e
...
- **Solutions appliqu├®es** : 
  ...
  - Mise ├á jour de tous les imports et r├®f├®rences dans le codebase
...
### Septembre 2025 - Syst├¿me de sauvegarde/chargement des configurations Enhanced Grid Layout (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Solutions appliqu├®es** : 
  ...
  - Renommage `absolute_position` ÔåÆ `manual_position` dans JSON pour coh├®rence s├®mantique
  - Filtrage des configurations par `model_name` pour isolation entre mod├¿les
  - Gestion automatique du passage "layoutOrder" ÔåÆ "mixed" lors du drag manuel
...
### Septembre 2025 - Int├®gration Enhanced Grid Layout pour les gameSpaces (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Solutions appliqu├®es** : 
  ...
  - Renommage syst├®matique pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout
  ...
  - Documentation mise ├á jour avec approche minimale
- **R├®sultat** : Syst├¿me Enhanced Grid Layout complet et int├®gr├®, architecture polymorphique am├®lior├®e, interface utilisateur intuitive, documentation ├á jour

### Septembre 2025 - Fonctionnalit├® de zoom pour les grilles (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Probl├¿mes rencontr├®s** : Positionnement incorrect des agents lors du zoom, calculs hexagonaux erron├®s, timing des mises ├á jour Qt
- **Solutions appliqu├®es** : 
  ...
  - Exemples organis├®s par complexit├® (ex_zoom_1 ├á ex_zoom_3)
- **R├®sultat** : Fonctionnalit├® de zoom compl├¿te et robuste pour tous types de grilles avec agents, documentation mise ├á jour

### 26/12/2024 - Syst├¿me de tooltips avanc├® (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Fichiers concern├®s** : 
  ...
  - `examples/syntax_examples/ex_tooltip_2.py` (exemple mis ├á jour)
  - `examples/syntax_examples/ex_tooltip_3.py` (nouveau exemple)
  - `FUTURE_PLAN.md` (mise ├á jour chantier termin├®)
...
### 26/12/2024 - Refactoring drag & drop des gameSpaces + corrections SGControlPanel + SGGrid (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
- **Description** : Refactoring complet du syst├¿me de drag & drop des gameSpaces, passage de QDrag ├á mouvement direct de la souris, corrections SGControlPanel et SGGrid pour compatibilit├®
- **Fichiers concern├®s** : 
  - `mainClasses/SGGameSpace.py` (refactoring drag & drop, passage QDrag ÔåÆ mouvement direct)
  - `mainClasses/SGControlPanel.py` (refactoring constructeur __init__, corrections mousePressEvent)
  - `mainClasses/SGPlayer.py` (mise ├á jour utilisation SGControlPanel)
  ...
  - `FUTURE_PLAN.md` (mise ├á jour chantier drag & drop termin├®)
...
- **Solutions appliqu├®es** : 
  ...
  - Mise ├á jour SGPlayer pour utiliser nouveau constructeur SGControlPanel
  - Correction SGGrid.mouseMoveEvent pour d├®l├®guer ├á super() quand isDraggable
...
### 26/12/2024 - Gestion de taille des gameSpaces + SGGameSpaceSizeManager (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Solutions appliqu├®es** : 
  ...
  - R├®duction hauteur SGUserSelector (50px ÔåÆ 25px + padding r├®duit)
...
### 26/12/2024 - Refactoring Model-View + corrections finales + documentation (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
- **Description** : Refactoring complet de l'architecture Model-View, corrections des m├®thodes dans SGEntityDef, r├®solution des bugs UI, et mise ├á jour de la documentation
- **Fichiers concern├®s** : 
  - `mainClasses/SGEntity.py` (renomm├® de SGEntityModel.py, classe de base)
  - `mainClasses/SGAgent.py` (h├®rite de SGEntity, d├®l├¿gue UI ├á SGAgentView)
  ...
  - `mainClasses/SGModel.py` (imports mis ├á jour, gestion phases)
  ...
  - `DEV_NOTES.md` (mise ├á jour ├®tat actuel)
  - `CONTEXT_SGE_FOR_CHATBOT.md` (r├¿gles critiques Model-View)
  - `tests/test_model_view_architecture.py` (imports mis ├á jour)
  - `tests/test_model_view_separation.py` (imports mis ├á jour)
  - `tests/test_entity_factory.py` (renomm├® de test_entity_def_factory.py)
  - `tests/test_sgagent_model_view_adaptation.py` (imports mis ├á jour)
...
- **Solutions appliqu├®es** : 
  ...
  - Renommage SGEntityDefFactory ÔåÆ SGEntityFactory pour clart├®
...
### 25/08/2025 - Refactoring Admin-to-super-player (TERMIN├ë)
- **Statut** : Ô£à Termin├® et merg├® sur main_candidate_release_august_2025
...
### 25/08/2025 - Am├®lioration API d├®l├®gation et tests (TERMIN├ë)
- **Statut** : Ô£à Termin├®
...
- **Fichiers concern├®s** : 
  ...
  - `mainClasses/SGTimeManager.py` (conversion automatique nomsÔåÆinstances)
...
### 26/12/2024 - Renommage SGEntityModel ÔåÆ SGEntity (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Fichiers concern├®s** :
  - `mainClasses/SGEntityModel.py` ÔåÆ `mainClasses/SGEntity.py` (renomm├®)
  - `mainClasses/SGAgent.py` (h├®ritage mis ├á jour)
  - `mainClasses/SGCell.py` (h├®ritage mis ├á jour)
  - `mainClasses/SGEntityFactory.py` (imports mis ├á jour)
  - `mainClasses/SGDashBoard.py` (r├®f├®rences de type mises ├á jour)
  - `mainClasses/SGDataRecorder.py` (imports mis ├á jour)
  - `mainClasses/SGTestGetData.py` (imports mis ├á jour)
  - `mainClasses/SGModel.py` (imports mis ├á jour)
  - `mainClasses/SGCellView.py` (imports mis ├á jour)
  - `mainClasses/SGFormatDataHistory.py` (imports mis ├á jour)
  - `tests/test_sgagent_model_view_adaptation.py` (imports mis ├á jour)
  - Documentation mise ├á jour (README_developer.md, DEV_NOTES.md, CONTEXT_SGE_FOR_CHATBOT.md)
- **R├®sultat** : Architecture parfaitement coh├®rente avec SGEntity comme classe de base pour SGAgent et SGCell

### 26/12/2024 - Syst├¿me de g├®n├®ration de catalogue de m├®thodes (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
### 26/12/2024 - Syst├¿me d'export d'ex├®cutables SGE (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
- **Description** : Cr├®ation d'un syst├¿me complet pour exporter des mod├¿les SGE en ex├®cutables Windows autonomes avec PyInstaller
- **Fichiers concern├®s** : 
  - `guide-export-exe.md` (nouveau - guide complet d'export)
  - `mainClasses/SGExeBuilder.py` (nouveau - classe pour cr├®ation d'ex├®cutables)
  - `examples/games/morpion_game.py` (test de validation)
  - `morpion_game.spec` (g├®n├®r├® - fichier de configuration PyInstaller)
  - `dist/MorpionGame.exe` (g├®n├®r├® - ex├®cutable final 143.7 MB)
- **Probl├¿mes rencontr├®s** : 
  - Modules manquants dans PyInstaller (logging.config, PyQt5.QtSvg, screeninfo, matplotlib.backends, pandas)
  - D├®pendances SGE non incluses automatiquement
  - Configuration PyInstaller complexe
- **Solutions appliqu├®es** : 
  - Cr├®ation de la classe `SGExeBuilder` avec 65 d├®pendances SGE incluses
  - G├®n├®ration automatique du fichier .spec avec template
  - Inclusion de toutes les ressources SGE (mainClasses, icon, images)
  - Guide complet avec exemples d'utilisation et d├®pannage
  - Support des options personnalis├®es (nom, ic├┤ne, ressources)
- **R├®sultat** : Syst├¿me complet d'export d'ex├®cutables avec classe SGE d├®di├®e, guide d├®taill├®, et ex├®cutable fonctionnel

### 26/12/2024 - Am├®lioration interface HTML du catalogue de m├®thodes (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
- **Fichiers concern├®s** : 
  ...
  - `FUTURE_PLAN.md` (mise ├á jour statut t├óche)
  - `CONTEXT_SGE_FOR_CHATBOT.md` (ajout section interface HTML)
  - `README_modeler.md` (mise ├á jour section Method Catalog)
...
- **Fonctionnalit├®s ajout├®es** : 
  ...
  - **Boutons de copie de syntaxe** pour les m├®thodes (­ƒôï sans param├¿tres, ­ƒôØ avec param├¿tres)
  - **Case ├á cocher** pour inclure/exclure le nom d'objet dans la syntaxe copi├®e
- **Probl├¿mes r├®solus** : 
  ...
  - M├®thodes commen├ºant par __ incluses (exclues maintenant)
  - Retours ├á la ligne dans descriptions de param├¿tres non pr├®serv├®s
...
### 26/12/2024 - Architecture Model-View (TERMIN├ë)
- **Statut** : Ô£à Termin├® et valid├®
...
### Fonctionnalit├® de zoom - TERMIN├ë Ô£à
...
- [x] Exemples organis├®s par complexit├® (ex_zoom_1.py ├á ex_zoom_3.py)
- [x] Documentation mise ├á jour (FUTURE_PLAN.md, README_developer.md, CONTEXT_SGE_FOR_CHATBOT.md)
- [x] Tests valid├®s avec tous types de grilles et agents

### Gestion de taille des gameSpaces - TERMIN├ë Ô£à
...
- [x] R├®duction hauteur SGUserSelector (50px ÔåÆ 25px + padding r├®duit)
...
- [x] ├ëlimination des tailles disproportionn├®es dans SGEndGameCondition
- [x] Tests avec exStep8.py, aGameExample.py, Delmoges_v3.py valid├®s
- [x] Mise ├á jour FUTURE_PLAN.md avec les avanc├®es

### Architecture Model-View - TERMIN├ë Ô£à
L'architecture Model-View est **compl├¿tement termin├®e** et valid├®e :
- [x] Architecture Model-View impl├®ment├®e et test├®e
- [x] Renommage SGCellModel ÔåÆ SGCell pour coh├®rence
- [x] Suppression SGEntity et renommage SGEntityModel ÔåÆ SGEntity
- [x] Renommage SGEntityDefFactory ÔåÆ SGEntityFactory
- [x] Documentation README_developer.md mise ├á jour
...
- [x] Documentation CONTEXT_SGE_FOR_CHATBOT.md mise ├á jour

### Refactoring drag & drop des gameSpaces - TERMIN├ë Ô£à
...
- [x] Mise ├á jour SGPlayer pour utiliser nouveau constructeur SGControlPanel
- [x] Correction SGGrid.mouseMoveEvent pour d├®l├®guer ├á super() quand isDraggable
...
- [x] Mise ├á jour FUTURE_PLAN.md avec le chantier termin├®

### Enhanced Grid Layout - TERMIN├ë Ô£à
...
- [x] Renommage syst├®matique pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout
- [x] Nettoyage complet des m├®thodes inutilis├®es et prints debug
- [x] Documentation mise ├á jour (README_developer.md, README_modeler.md, FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
- [x] Tests valid├®s avec ex_enhanced_grid_layout_1.py

### Syst├¿me de sauvegarde/chargement des configurations Enhanced Grid Layout - TERMIN├ë Ô£à
...
- [x] Gestion automatique des transitions entre types de position (layoutOrder ÔåÆ mixed ÔåÆ absolute)
...
- [x] Renommage absolute_position ÔåÆ manual_position pour coh├®rence s├®mantique
- [x] Nettoyage complet des prints debug et m├®thodes obsol├¿tes
- [x] Documentation mise ├á jour (README_modeler.md, CONTEXT_SGE_FOR_CHATBOT.md)
- [x] Tests valid├®s avec exemples multiples et mod├¿les complexes (CarbonPolis.py)

### Refactoring SGModel + Extraction MQTT - TERMIN├ë Ô£à
...
- [x] Mise ├á jour toutes r├®f├®rences MQTT vers SGMQTTManager
- [x] Ajout m├®thodes placeholder pour visibilit├® IDE outline
- [x] Documentation mise ├á jour (FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
- [x] Tests valid├®s avec MQTT_GameExample_Player1.py
- [x] Code parfaitement organis├® selon conventions SGE

### Syst├¿me de g├®n├®ration de catalogue de m├®thodes - TERMIN├ë Ô£à
...
- [x] Gestion de l'h├®ritage r├®cursif (SGCell ÔåÆ SGEntity ÔåÆ AttributeAndValueFunctionalities)
...
### Syst├¿me d'export d'ex├®cutables SGE - TERMIN├ë Ô£à
Le syst├¿me d'export d'ex├®cutables SGE est **compl├¿tement termin├®** et valid├® :
- [x] Cr├®ation de la classe SGExeBuilder.py avec 65 d├®pendances SGE incluses
- [x] G├®n├®ration automatique du fichier .spec avec template PyInstaller
- [x] Inclusion de toutes les ressources SGE (mainClasses, icon, images)
- [x] Guide complet guide-export-exe.md avec exemples et d├®pannage
- [x] Support des options personnalis├®es (nom, ic├┤ne, ressources)
- [x] R├®solution des modules manquants (logging.config, PyQt5.QtSvg, screeninfo, matplotlib.backends, pandas)
- [x] Test valid├® avec morpion_game.py ÔåÆ MorpionGame.exe (143.7 MB)
- [x] Ex├®cutable fonctionnel et autonome sur Windows
- [x] Syst├¿me pr├¬t pour export de tous les mod├¿les SGE
...
- [ ] Extension SGGameSpaceSizeManager ├á d'autres types de widgets
...
- **Solution** : 
  ...
  7. Mise ├á jour toutes r├®f├®rences MQTT vers SGMQTTManager
  8. Ajout m├®thodes placeholder pour visibilit├® IDE outline
  9. Documentation mise ├á jour (FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- **Solution** : 
  ...
  7. Gestion automatique des transitions entre types de position (layoutOrder ÔåÆ mixed lors du drag)
  ...
  10. Renommage absolute_position ÔåÆ manual_position pour coh├®rence s├®mantique
  11. Nettoyage complet des prints debug et m├®thodes obsol├¿tes
  12. Documentation mise ├á jour avec approche minimale (README_modeler.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- **Solution** : 
  ...
  8. Renommage syst├®matique pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout
  9. Nettoyage complet des m├®thodes inutilis├®es et prints debug
  10. Documentation mise ├á jour avec approche minimale (README_developer.md, README_modeler.md, FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- **Chat utilis├®** : Claude Sonnet 4 (Cursor)
- **Impact** : Syst├¿me Enhanced Grid Layout complet et int├®gr├®, architecture polymorphique am├®lior├®e, interface utilisateur intuitive, documentation ├á jour
...
- **Solution** : 
  ...
  9. Cr├®ation exemples organis├®s par complexit├® (ex_zoom_1.py ├á ex_zoom_3.py)
  10. Mise ├á jour documentation compl├¿te (FUTURE_PLAN.md, README_developer.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- **Chat utilis├®** : Claude Sonnet 4 (Cursor)
- **Impact** : Fonctionnalit├® de zoom compl├¿te et robuste pour tous types de grilles avec agents, documentation mise ├á jour
...
- **Solution** : 
  ...
  5. Mise ├á jour SGPlayer pour utiliser nouveau constructeur SGControlPanel
  6. Correction SGGrid.mouseMoveEvent pour d├®l├®guer ├á super() quand isDraggable
...
- **Solution** : 
  ...
  6. R├®duction hauteur `SGUserSelector` (50px ÔåÆ 25px + padding r├®duit)
  ...
  10. ├ëlimination des tailles disproportionn├®es dans `SGEndGameCondition`
...
- **Solution** : 
  ...
  2. Renommage SGCellModel ÔåÆ SGCell et SGEntityModel ÔåÆ SGEntity pour coh├®rence
  3. Renommage SGEntityDefFactory ÔåÆ SGEntityFactory pour clart├®
...
- **Solution** : 
  ...
  3. Correction newAgentOnCell() pour ajouter seulement le mod├¿le ├á self.entities
  4. Suppression des m├®thodes dupliqu├®es dans SGEntityDef
  5. Protection RuntimeError avec try-except dans deleteEntity() pour vues d├®j├á supprim├®es
...
- **Solution** : 
  ...
  2. Gestion de l'h├®ritage r├®cursif (SGCell ÔåÆ SGEntity ÔåÆ AttributeAndValueFunctionalities)
...
### 26/12/2024 - Syst├¿me d'export d'ex├®cutables SGE (MAJOR)
- **Description** : Cr├®ation d'un syst├¿me complet pour exporter des mod├¿les SGE en ex├®cutables Windows autonomes avec PyInstaller
- **Solution** : 
  1. Cr├®ation de la classe SGExeBuilder.py avec 65 d├®pendances SGE incluses
  2. G├®n├®ration automatique du fichier .spec avec template PyInstaller
  3. Inclusion de toutes les ressources SGE (mainClasses, icon, images)
  4. Guide complet guide-export-exe.md avec exemples et d├®pannage
  5. Support des options personnalis├®es (nom, ic├┤ne, ressources)
  6. R├®solution des modules manquants (logging.config, PyQt5.QtSvg, screeninfo, matplotlib.backends, pandas)
  7. Test valid├® avec morpion_game.py ÔåÆ MorpionGame.exe (143.7 MB)
  8. Ex├®cutable fonctionnel et autonome sur Windows
  9. Syst├¿me pr├¬t pour export de tous les mod├¿les SGE
- **Fichiers modifi├®s** : guide-export-exe.md (nouveau), mainClasses/SGExeBuilder.py (nouveau), morpion_game.spec (g├®n├®r├®), dist/MorpionGame.exe (g├®n├®r├®)
- **Chat utilis├®** : Claude Sonnet 4 (Cursor)
- **Impact** : Syst├¿me complet d'export d'ex├®cutables avec classe SGE d├®di├®e, guide d├®taill├®, et ex├®cutable fonctionnel
...
- **Solution** : 
  ...
  3. Renommage SGCellModel ÔåÆ SGCell pour coh├®rence nomenclature
...
- **Solution** : 
  ...
  2. D├®l├®gation cr├®ation vue ├á SGEntityFactory
...
- **Solution** : 
  ...
  3. D├®l├®gation activation control panels ├á SGTimeManager.updateControlPanelsForCurrentPhase
...
- **Description** : Erreurs d'imports apr├¿s suppression SGCellModel.py et r├®f├®rences ├á SGCell
- **Solution** : 
  1. Replace all SGCellModel ÔåÆ SGCell dans tous les fichiers
  2. Mise ├á jour tous les imports pour pointer vers mainClasses.SGCell
  3. Suppression r├®f├®rences obsol├¿tes et m├®thodes temporaires
- **Fichiers modifi├®s** : 13 fichiers avec imports mis ├á jour

### 25/08/2025 - Am├®lioration API d├®l├®gation et conversion noms de joueurs
- **Description** : Les m├®thodes de d├®l├®gation dans SGModel ne prenaient pas en compte tous les param├¿tres disponibles et la conversion automatique des noms de joueurs ├®tait limit├®e ├á 'Admin'
- **Solution** : 
...
- **Impact** : 
  - ├ëtat explicite et coh├®rent des gameSpaces
  - M├®thodes setToAbsolute(), setToLayoutOrder(), setToMixed() pour gestion d'├®tat
  - Transitions automatiques (layoutOrder ÔåÆ mixed lors du drag)
  - Restauration intelligente des layoutOrder originaux
  - ├ëlimination de la d├®tection dynamique complexe
...
- **Impact** : 
  - Am├®lioration significative des performances (lookups O(n) ÔåÆ O(1))
...
### Septembre 2025 - Renommage absolute_position ÔåÆ manual_position
...
### Septembre 2025 - Renommage syst├®matique pID ÔåÆ layoutOrder
- **Contexte** : Terminologie confuse avec "pID" peu compr├®hensible
- **D├®cision prise** : Renommage syst├®matique pID ÔåÆ layoutOrder dans tout le code et la documentation
- **Impact** : 
...
### Septembre 2025 - Renommage EGL ÔåÆ Enhanced Grid Layout
...
- **Contexte** : Positionnement incorrect des agents lors du zoom, probl├¿me de timing entre mises ├á jour des cellules et agents
...
- **D├®cision prise** : Organiser les exemples du plus simple au plus complexe (ex_zoom_1.py ├á ex_zoom_3.py)
- **Impact** : 
...
- **D├®cision prise** : Cr├®er une classe d├®di├®e `SGGameSpaceSizeManager` pour centraliser la logique de sizing, similaire ├á `SGAspect` pour les styles
- **Impact** : 
...
- **Impact** : 
  ...
  - ├ëlimination des tailles disproportionn├®es
...
- **Impact** : 
  ...
  - Word wrapping intelligent (seuil ├á 50 caract├¿res)
...
- **Impact** : ├ëvite les erreurs courantes pour les futurs chatbots
...
- **D├®cision prise** : 
  ...
  4. `method='cardinal'` ÔåÆ `method='direction'` (plus clair)
...
- **D├®cision prise** : Renommage SGCellModel ÔåÆ SGCell pour coh├®rence
...
- **D├®cision prise** : 
  ...
  2. ├ëtendre la conversion automatique des noms de joueurs en instances (pas seulement 'Admin')
- **Impact** : 
...
- **Avantage** : ├ëtat coh├®rent et pr├®visible, ├®limination de la d├®tection dynamique complexe
...
- **Avantage** : Am├®lioration significative des performances, r├®duction des it├®rations co├╗teuses

### Septembre 2025 - Renommage s├®mantique pour clart├®
- **Convention** : Renommer les termes techniques pour refl├®ter l'usage r├®el
- **Exemples** : `absolute_position` ÔåÆ `manual_position`, support r├®trocompatibilit├®
...
- **Exemples** : `moveToCoords()` ÔåÆ `layoutOrder="manual_position"`, tooltips informatifs
...
- **Exemples** : pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout, "fixed_position" ÔåÆ "manual_position"
...
- **Avantage** : ├ëvite les probl├¿mes de timing entre mises ├á jour Qt
...
- **Avantage** : ├ëvite les erreurs courantes pour les futurs chatbots
...
- **Avantage** : Documentation automatique et ├á jour, d├®couverte des m├®thodes facilit├®e

### 26/12/2024 - Syst├¿me d'export d'ex├®cutables SGE
- **Convention** : Utiliser SGExeBuilder.py pour cr├®er des ex├®cutables Windows autonomes
- **Exemples** : `builder.build_exe("model.py")`, `builder.build_exe("model.py", output_name="MonJeu", icon="icone.ico")`
- **Avantage** : Export simple et automatis├®, ex├®cutables autonomes avec toutes les d├®pendances
...
- **Convention** : Remonter r├®cursivement dans la cha├«ne d'h├®ritage pour extraire toutes les m├®thodes
- **Exemples** : SGCell ÔåÆ SGEntity ÔåÆ AttributeAndValueFunctionalities
- **Avantage** : Catalogue complet avec toutes les m├®thodes h├®rit├®es

### 26/12/2024 - Extension palette couleurs Qt
- **Convention** : ├ëtendre la palette Qt avec des couleurs personnalis├®es dans SGExtensions.py
...
- **R├®sultats** : 
  ...
  - Gestion automatique des transitions entre types de position (layoutOrder ÔåÆ mixed lors du drag)
  - Optimisations de performance avec cache gameSpaces_cache pour lookups O(1)
  - Filtrage des configurations par model_name pour isolation entre mod├¿les
  - Renommage absolute_position ÔåÆ manual_position pour coh├®rence s├®mantique
  - Nettoyage complet des prints debug et m├®thodes obsol├¿tes
  - Documentation mise ├á jour avec approche minimale (README_modeler.md, CONTEXT_SGE_FOR_CHATBOT.md)
- **Fichiers modifi├®s** : `SGLayoutConfigManager.py` (nouveau), `SGLayoutConfigSaveDialog.py` (nouveau), `SGLayoutConfigManagerDialog.py` (nouveau), `SGLayoutOrderTableDialog.py`, `SGEnhancedGridLayout.py`, `SGGameSpace.py`, `SGModel.py`, `README_modeler.md`, `CONTEXT_SGE_FOR_CHATBOT.md`
- **Dur├®e** : Session compl├¿te de d├®veloppement
...
- **R├®sultats** : 
  ...
  - Renommage syst├®matique pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout
  - Nettoyage complet des m├®thodes inutilis├®es et prints debug
  - Documentation mise ├á jour avec approche minimale (README_developer.md, README_modeler.md, FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
- **Fichiers modifi├®s** : `SGEnhancedGridLayout.py` (nouveau), `SGLayoutOrderTableDialog.py` (nouveau), `SGAbstractLayout.py`, `SGVerticalLayout.py`, `SGHorizontalLayout.py`, `SGGridLayout.py`, `SGGameSpace.py`, `SGModel.py`, `ex_enhanced_grid_layout_1.py`, documentation compl├¿te
- **Dur├®e** : Session compl├¿te de d├®veloppement
...
- **R├®sultats** : 
  ...
  - Nettoyage complet des prints de debug
  - Cr├®ation exemples organis├®s par complexit├® (ex_zoom_1.py ├á ex_zoom_3.py)
  - Mise ├á jour documentation compl├¿te (FUTURE_PLAN.md, README_developer.md, CONTEXT_SGE_FOR_CHATBOT.md)
- **Fichiers modifi├®s** : `SGGrid.py`, `SGCellView.py`, `SGAgentView.py`, `SGAgent.py`, `ex_zoom_1.py`, `ex_zoom_2.py`, `ex_zoom_3.py`, `FUTURE_PLAN.md`, `README_developer.md`, `CONTEXT_SGE_FOR_CHATBOT.md`
- **Dur├®e** : Session compl├¿te de d├®veloppement
...
### 26/12/2024 - Refactoring drag & drop des gameSpaces (MAJOR)
- **Ordinateur** : Windows 10 (nbecu)
- **Sujet principal** : Refactoring complet du syst├¿me de drag & drop des gameSpaces, passage de QDrag ├á mouvement direct de la souris, corrections SGControlPanel et SGGrid
- **R├®sultats** : 
  - Abandon complet du syst├¿me QDrag au profit d'un mouvement direct bas├® sur global mouse position
  - Impl├®mentation du comportement hotspot intuitif (point cliqu├® reste sous curseur)
  - Refactoring SGControlPanel pour utiliser constructeur __init__ standard
...
- **R├®sultats** : 
  ...
  - ├ëlimination des tailles disproportionn├®es dans `SGEndGameCondition`
- **Fichiers modifi├®s** : `SGGameSpaceSizeManager.py` (nouveau), `SGGameSpace.py`, `SGTextBox.py`, `SGEndGameRule.py`, `SGEndGameCondition.py`, `SGUserSelector.py`, `SGModel.py`, `FUTURE_PLAN.md`
- **Dur├®e** : Session compl├¿te de d├®veloppement
- **Commits** : Multiple commits avec push sur refactor/model-view-separation
...
- **Ordinateur** : Windows 10 (nbecu)
- **Sujet principal** : Refactoring complet de l'architecture Model-View, am├®lioration des m├®thodes de d├®placement, cr├®ation d'exemples, et mise ├á jour de la documentation
- **R├®sultats** : 
  ...
  - Mise ├á jour compl├¿te de la documentation (README_developer.md, README_modeler.md)
...
- **R├®sultats** : 
  ...
  - Renommage SGCellModel ÔåÆ SGCell pour coh├®rence
...
- **Fichiers modifi├®s** : 
  ...
  - mainClasses/SGTimeManager.py (conversion nomsÔåÆinstances)
...
- **R├®sultats** : 
  ...
  - Gestion de l'h├®ritage r├®cursif (SGCell ÔåÆ SGEntity ÔåÆ AttributeAndValueFunctionalities)
...
### 26/12/2024 - Syst├¿me d'export d'ex├®cutables SGE (MAJOR)
- **Ordinateur** : Windows 10 (nbecu)
- **Sujet principal** : Cr├®ation d'un syst├¿me complet pour exporter des mod├¿les SGE en ex├®cutables Windows autonomes avec PyInstaller
- **R├®sultats** : 
  - Cr├®ation de la classe SGExeBuilder.py avec 65 d├®pendances SGE incluses
  - G├®n├®ration automatique du fichier .spec avec template PyInstaller
  - Inclusion de toutes les ressources SGE (mainClasses, icon, images)
  - Guide complet guide-export-exe.md avec exemples et d├®pannage
  - Support des options personnalis├®es (nom, ic├┤ne, ressources)
  - R├®solution des modules manquants (logging.config, PyQt5.QtSvg, screeninfo, matplotlib.backends, pandas)
  - Test valid├® avec morpion_game.py ÔåÆ MorpionGame.exe (143.7 MB)
  - Ex├®cutable fonctionnel et autonome sur Windows
  - Syst├¿me pr├¬t pour export de tous les mod├¿les SGE
- **Fichiers modifi├®s** : guide-export-exe.md (nouveau), mainClasses/SGExeBuilder.py (nouveau), morpion_game.spec (g├®n├®r├®), dist/MorpionGame.exe (g├®n├®r├®)
- **Dur├®e** : Session compl├¿te de d├®veloppement
- **Commits** : Multiple commits avec push sur main
...
- D├®cembre 2024 : Mise ├á jour toutes r├®f├®rences MQTT vers SGMQTTManager
- D├®cembre 2024 : Ajout m├®thodes placeholder pour visibilit├® IDE outline
- D├®cembre 2024 : Documentation mise ├á jour (FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- Septembre 2025 : Renommage absolute_position ÔåÆ manual_position pour coh├®rence s├®mantique dans JSON
- Septembre 2025 : Filtrage des configurations par model_name pour isolation entre mod├¿les
- Septembre 2025 : Gestion automatique des transitions entre types de position (layoutOrder ÔåÆ mixed lors du drag)
...
- Septembre 2025 : Documentation mise ├á jour avec approche minimale (README_modeler.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- Septembre 2025 : Support positionnement manuel avec layoutOrder="manual_position"
- Septembre 2025 : Renommage syst├®matique pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout
...
- Septembre 2025 : Documentation mise ├á jour avec approche minimale (README_developer.md, README_modeler.md, FUTURE_PLAN.md, CONTEXT_SGE_FOR_CHATBOT.md)
- Septembre 2025 : Fonctionnalit├® de zoom compl├¿te (SGGrid.py, SGCellView.py, SGAgentView.py, SGAgent.py, ex_zoom_1.py ├á ex_zoom_3.py)
...
- Septembre 2025 : Documentation mise ├á jour (FUTURE_PLAN.md, README_developer.md, CONTEXT_SGE_FOR_CHATBOT.md)
...
- 26/12/2024 : Documentation README_modeler.md mise ├á jour
- 26/12/2024 : Architecture Model-View compl├¿tement impl├®ment├®e (15+ fichiers)
- 26/12/2024 : Renommage SGCellModel ÔåÆ SGCell pour coh├®rence nomenclature
- 26/12/2024 : Documentation README_developer.md mise ├á jour avec section Model-View
...
- 26/12/2024 : Syst├¿me d'export d'ex├®cutables SGE (guide-export-exe.md, mainClasses/SGExeBuilder.py, dist/MorpionGame.exe)
- 26/12/2024 : Classe SGExeBuilder avec 65 d├®pendances SGE incluses et g├®n├®ration automatique .spec
- 26/12/2024 : R├®solution modules manquants PyInstaller (logging.config, PyQt5.QtSvg, screeninfo, matplotlib.backends, pandas)
- 26/12/2024 : Guide complet d'export avec exemples et d├®pannage
...
- D├®cembre 2024 : La documentation doit ├¬tre mise ├á jour simultan├®ment avec le code
...
- Septembre 2025 : Le renommage s├®mantique (absolute_position ÔåÆ manual_position) am├®liore la coh├®rence et la compr├®hensibilit├® du code
- Septembre 2025 : Le filtrage des configurations par model_name est essentiel pour l'isolation entre mod├¿les
- Septembre 2025 : La gestion automatique des transitions entre types de position (layoutOrder ÔåÆ mixed) am├®liore l'exp├®rience utilisateur
...
- Septembre 2025 : Le renommage syst├®matique des termes techniques (pID ÔåÆ layoutOrder, EGL ÔåÆ Enhanced Grid Layout) am├®liore la compr├®hensibilit├® du code
...
- Septembre 2025 : La documentation doit ├¬tre mise ├á jour simultan├®ment avec le code
...
- 26/12/2024 : SGGrid.mouseMoveEvent doit d├®l├®guer ├á super() quand isDraggable pour utiliser le nouveau syst├¿me
...
- 26/12/2024 : L'API transparente pour modelers est possible gr├óce ├á la d├®l├®gation automatique
...
- 26/12/2024 : Le syst├¿me d'export d'ex├®cutables SGE n├®cessite une classe d├®di├®e SGExeBuilder pour centraliser la logique PyInstaller
- 26/12/2024 : L'inclusion de 65 d├®pendances SGE dans PyInstaller est essentielle pour des ex├®cutables fonctionnels
- 26/12/2024 : La g├®n├®ration automatique du fichier .spec avec template am├®liore la maintenabilit├® et la coh├®rence
- 26/12/2024 : Le guide complet d'export avec exemples et d├®pannage facilite l'adoption par les utilisateurs
- 26/12/2024 : La r├®solution des modules manquants PyInstaller (logging.config, PyQt5.QtSvg, screeninfo, matplotlib.backends, pandas) est critique
- 26/12/2024 : L'ex├®cutable autonome de 143.7 MB est normal et inclut toutes les d├®pendances Python et SGE

### Questions en suspens
- Comment optimiser la performance du syst├¿me de persistance pour de tr├¿s grandes configurations ?
- Faut-il ├®tendre le syst├¿me de sauvegarde/chargement ├á d'autres types de layouts SGE ?
...
- Faut-il ├®tendre le syst├¿me de zoom ├á d'autres types de widgets SGE ?
...
- Faut-il ├®tendre le syst├¿me de drag & drop ├á d'autres types de widgets SGE ?
...
- Faut-il ├®tendre SGGameSpaceSizeManager ├á d'autres types de widgets ?
...
- Faut-il ├®tendre l'architecture Model-View ├á d'autres entit├®s SGE ?
- Comment am├®liorer la d├®tection des tests pytest dans Cursor/IDE ?
- Faut-il ├®tendre la conversion automatique des noms ├á d'autres parties de l'API ?
- Comment optimiser la performance du syst├¿me de g├®n├®ration de catalogue pour de tr├¿s grandes codebases ?
- Faut-il ├®tendre le syst├¿me de g├®n├®ration de catalogue ├á d'autres types de documentation ?
...
- Comment optimiser la taille des ex├®cutables SGE pour r├®duire l'espace disque ?
- Faut-il ├®tendre le syst├¿me d'export ├á d'autres plateformes (Linux, macOS) ?
- Comment g├®rer la migration des mod├¿les existants vers le nouveau syst├¿me d'export ?
- Faut-il cr├®er d'autres exemples pour les nouvelles fonctionnalit├®s d'export ?
- Comment optimiser la performance de la g├®n├®ration d'ex├®cutables pour de gros mod├¿les ?
- Faut-il impl├®menter des raccourcis clavier pour l'export d'ex├®cutables ?
- Comment g├®rer l'export avec des mod├¿les tr├¿s complexes (multiples d├®pendances) ?
- Faut-il ajouter des indicateurs visuels du statut de g├®n├®ration d'ex├®cutable ?

---

## Instructions pour la mise ├á jour

### Avant de changer d'ordinateur :
1. **Commitez** vos changements sur GitHub
2. **Mettez ├á jour** ce fichier avec l'├®tat actuel
